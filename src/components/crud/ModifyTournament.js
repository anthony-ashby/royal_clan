...
const ModifyTournament = ({
  ...
  const [bodyText, setBodyText] = useState(tournament.body);
  ...
  const [modifyTournamentObject, setmodifyTournamentObject] = React.useState({
    dateCreated: "",
    dateModified: "",
    dateArchived: "",
    createdBy: "",
  ...
  useEffect(() => {
    if (tournament) {
      const tournamentSections = JSON.parse(tournament.sections);
      setmodifyTournamentObject(tournament);
      setSections(tournamentSections);
      setSectionCount(tournamentSections.length);
  ...
  function handleModifySubmit(event) {
    ...
    const updateTournamentObj = {
      _id: modifyTournamentObject._id,
      dateCreated: modifyTournamentObject.dateCreated,
      dateModified: new Date().toISOString(),
      createdBy: modifyTournamentObject.createdBy,
      modifiedBy: currentUser.email,
      title: modifyTournamentObject.title,
      imageURL: uploadedImageURL ? uploadedImageURL : null,
      body: bodyText,
      type: modifyTournamentObject.type,
      sections: JSON.stringify(sections),
    };

    const updateTournament = async () => {
      if (updateTournamentObj.title !== "" && updateTournamentObj.body !== "") {
        try {
          await fetch("/.netlify/functions/updateAnnouncement", {
            method: "PUT",
            body: JSON.stringify(updateTournamentObj),
          });
        } catch (err) {
          console.log(err);
        }
  ...
  function handleModifyFormTextFieldChange(event) {
    ...
    setmodifyTournamentObject((prevInput) => {
      return {
        ...prevInput,
        [name]: value,
      };
    ...
    <>
      <form noValidate autoComplete="off">
        <TextField
          ...
          name="title"
          size="small"
          value={modifyTournamentObject.title}
        ...
        {mediaType === "video" ? (
          <TextField
            ...
            name="videoURL"
            size="small"
            value={modifyTournamentObject.video}
            onChange={handleModifyFormTextFieldChange}
          />
        ) : null}
          ...
          <Row className={"no-gutters"}>
            <Col xl={6} xs={12}>
              <ColorButtonRed
                ...
                color="primary"
                disabled={loading}
                onClick={() => handleCloseModifyTournament()}
                style={{ width: "90%" }}
              >
                Cancel
              </ColorButtonRed>
